        ...
        <div class="d-flex align-items-baseline flex-wrap mr-5">
          ...
          <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            <li class="breadcrumb-item text-muted">
              <a href="" class="text-muted">Blog Cateory</a>
            </li>
          </ul>
          <!--end::Breadcrumb-->
        </div>
      ...
      <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
          ...
          <div class="col-xs-6 text-xs-right">
            <a href="/admin/blog_category/create" type="button" class="btn admin_ad_btn_red mr-2">ADD Blog Category</a>
          </div>
        </div>
        <div class="card-body">
          <!--begin: Search Form-->
          ...
          <table class="datatable datatable-bordered datatable-head-custom table-bordered text-center" id="kt_datatable">
            <thead>
              <tr>
                ...
                <th title="Field #2">Blog Category Name</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <a href="/admin/blog_category/{{ $blogCategory->id }}/edit"><i class="fa fa-edit ml-2"></i></a>
                  <a href="javascript:void(0)" onclick="deletetIsArchive('{{$blogCategory->id}}')">
...
<script>
  ...
  function deletetIsArchive(blog_category_id) {
    // let objProject = JSON.parse(project);
    console.log("deletetIsArchive -> blog_category_id -> ", blog_category_id);
    // ShowSweetAlert({
    //   title: 'Shortlisted!',
    //   text: 'Candidates are successfully shortlisted!',
    //   icon: 'success'
    // });
    if (parseInt(blog_category_id)) {
      ShowSweetAlertConfirm({
        title: "Are you sure ?",
        text: "You want to delete this blog category!",
        icon: "warning",
        showDenyButton: true,
        ...
        if (result.isConfirmed) {
          let requestRoute = "/admin/blog_category/delete";
          let requestParams = {
          ...
          CallLaravelAction(requestRoute, requestParams, function(response) {
            if (response.status) {
              ...
              if (typeof response.error !== "undefined") {
                if (typeof response.error.blog_category_id !== "undefined") {
                  ErrorMsg = response.error.blog_category_id;
                }
              }
              let SweetAlertParams = {
                icon: "error",
