        ...
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
          <div class="col-xs-6">
            <div class="card-title">
              <h3 class="card-label">Progress Details</h3>
            </div>
          </div>
          <div class="col-xs-6 text-xs-right">
            <a href="/admin/progress/create" type="button" class="btn admin_ad_btn_red mr-2">Add Progress</a>
          </div>
        </div>
        <div class="card-body">
          <!--begin: Search Form-->
          ...
          <table class="datatable datatable-bordered datatable-head-custom table-bordered text-center" id="kt_datatable">
            ...
            <tbody>
              ...
              <tr>
                <td>{{ $loop->index + 1 }}</td>
                <td>{{ $progress_data->progress_status_name }}</td>
                <td>
                  <a href="/admin/progress/{{ $progress_data->id }}/edit"><i class="fa fa-edit ml-2"></i></a>
                  <a onclick="deletetIsArchive('{{$progress_data->id}}')" href="javascript:void(0)">
...
<script>
  ...
  function deletetIsArchive(progress_id) {
    // let objProject = JSON.parse(project);
    console.log("deletetIsArchive -> project -> id -> ", progress_id);
    // ShowSweetAlert({
    //   title: 'Shortlisted!',
    //   text: 'Candidates are successfully shortlisted!',
    //   icon: 'success'
    // });
    if (parseInt(progress_id)) {
      ShowSweetAlertConfirm({
        ...
        if (result.isConfirmed) {
          let requestRoute = "/admin/progress/delete";
          let requestParams = {
