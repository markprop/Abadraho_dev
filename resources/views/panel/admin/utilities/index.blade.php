        ...
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
          <div class="col-xs-6">
            <div class="card-title">
              <h3 class="card-label">Utilities Details</h3>
            </div>
          </div>
          ...
          <table class="datatable datatable-bordered datatable-head-custom table-bordered text-center" id="kt_datatable">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <a href="/admin/utilities/{{ $utility->id }}/edit"><i class="fa fa-edit ml-2"></i></a>
                  <a onclick="deletetIsArchive('{{ $utility->id }}')" href="javascript:void(0)">
...
<script>
  function deletetIsArchive(utility_id) {
    // console.log("deletetIsArchive -> User -> id -> ", utility_id);
    if (parseInt(utility_id)) {
      ShowSweetAlertConfirm({
        ...
        if (result.isConfirmed) {
          let requestRoute = "/admin/utilities/delete";
          let requestParams = {
          ...
          CallLaravelAction(requestRoute, requestParams, function(response) {
            if (response.status) {
              ...
              if (typeof response.error !== "undefined") {
                if (typeof response.error.utility_id !== "undefined") {
                  ErrorMsg = response.error.utility_id;
                }
              }
              let SweetAlertParams = {
                icon: "error",
